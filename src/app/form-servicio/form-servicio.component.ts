...
export class FormServicioComponent {
  ...
  enviar() {
    this._apiService
      .postGuardarServicio(JSON.stringify(this.formularioServicio.value))
      .subscribe(
        (response) => {
          Swal.fire({
            title: 'Guardado!',
  ...
  hasErrors(controlName: string) {
    return (
      this.formularioServicio.get(controlName)?.hasError('required') &&
      this.formularioServicio.get(controlName)?.touched
    );
  }
